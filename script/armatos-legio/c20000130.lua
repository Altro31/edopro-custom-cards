...
function s.cfilter(c)
	return c:IsFaceup() and c:IsSetCard(CARDS_A_LEGIO) and c:IsLinkMonster() and c:IsLinked()
end
	--If monster effect or spell/trap card is activated
function s.condition(e,tp,eg,ep,ev,re,r,rp)
	if not Duel.IsExistingMatchingCard(s.cfilter,tp,LOCATION_MZONE,0,1,nil) then return false end
	return Duel.IsChainNegatable(ev) and (re:IsActiveType(TYPE_MONSTER) or re:IsHasType(EFFECT_TYPE_ACTIVATE)) and rp==1-tp
end
	--Activation legality
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	if Duel.NegateActivation(ev) and re:GetHandler():IsRelateToEffect(re) then
        ...
        if #g>0 then
            ...
            local tg=g:FilterSelect(tp,1,1,nil)
            if #tg>0 then
...
function s.drcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return rp==1-tp and c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsPreviousPosition(POS_FACEDOWN)
end
